...
const Profile = () => {
  ...
  const fetchProfile = useCallback(async () => {
    ...
    try {
      ...
      if (res.data.success) {
        ...
        // --- BUG FIX PART 1: LOAD SKILLS INTO INPUT ---
        // Convert skills array to a string and set it in the state
        // for the input field.
    ...
    try {
      ...
      if (imageFile) {
        ...
        const imgRes = await axios.post(
          `${backendUrl}/api/profile/upload-picture`,
          fd
        );
      ...
      const payload = {
        ...profileData,
        // --- BUG FIX PART 2: SAVE SKILLS CORRECTLY ---
        // Convert the comma-separated string from the input back to an
        // array of strings, trimming whitespace and removing empty entries.
        skills: skillsInput
          .split(",")
          .map((s) => s.trim())
          .filter(Boolean),
        ...
        socialLinks: {
          ...
          portfolio: normalizeUrl(profileData.socialLinks?.portfolio),
        },
      };
  ...
  const handleSendRequest = async () => {
    try {
      const res = await axios.post(
        `${backendUrl}/api/friends/requests/send/${userId}`
      );
  ...
  const handleWithdrawRequest = async () => {
    try {
      const res = await axios.delete(
        `${backendUrl}/api/friends/requests/withdraw/${userId}`
      );
      toast.success(res.data.message);
      fetchProfile(); // Refetch to update button state
    } catch (err) {
    ...
    switch (friendship?.status) {
      case "friends":
        return (
          <div className="flex items-center">
            <button
              type="button"
              className="px-5 py-2.5 bg-green-500 text-white font-semibold rounded-lg cursor-default"
            >
              Friends
            </button>
            <button
      ...
      case "request_sent":
        return (
          <div className="flex flex-col items-center gap-2">
            <button
              type="button"
              className="px-5 py-2.5 bg-gray-400 text-white font-semibold rounded-lg cursor-default w-full"
            >
              Request Sent
            </button>
            <button
      ...
      case "request_received":
        return (
          <Link
            to="/friends"
            className="px-5 py-2.5 bg-yellow-500 text-white font-semibold rounded-lg shadow-sm hover:bg-yellow-600"
          >
            Respond to Request
          </Link>
        );
      default:
        return (
          <button
            ...
            className="px-5 py-2.5 bg-indigo-600 text-white font-semibold rounded-lg shadow-sm hover:bg-indigo-700"
          >
            Add Friend
          </button>
        );
  ...
  if (!profileData)
    return (
      <div className="min-h-screen bg-slate-50 text-center pt-40">
        <h1 className="text-2xl font-bold">Profile Not Found</h1>
      </div>
    );

  const inputClasses =
    "mt-1 block w-full rounded-md border-0 bg-white py-1.5 text-gray-900 shadow-sm ring-1 ring-inset ring-gray-300 placeholder:text-gray-400 focus:ring-2 focus:ring-inset focus:ring-indigo-600";
  ...
  return (
    <div className="min-h-screen bg-slate-50">
      ...
      <div className="container mx-auto max-w-4xl pt-24 sm:pt-28 p-4 sm:p-6">
        <Link
          to={isOwnProfile ? "/dashboard" : "/skill-exchange"}
            ...
            <div className="flex flex-col md:flex-row items-center gap-6">
              ...
              <div className="flex-1 text-center md:text-left">
                ...
                <p className="mt-1 text-md text-gray-500">
                  {profileData.collegeName || "College not specified"}
                </p>
              ...
              <div className="mt-4 md:mt-0">{renderFriendshipButton()}</div>
            </div>
          </div>
              ...
              <div className="p-6 grid grid-cols-1 md:grid-cols-3 gap-6 items-center">
                ...
                <div className="md:col-span-1">
                  ...
                  <p className="mt-1 text-sm text-gray-500">
                    Recognitions earned from the community.
                  </p>
                </div>
                  ...
                  {profileData.badges.map((badge, idx) => (
                    ...
                    >
                      ...
                      <svg
                        ...
                        viewBox="0 0 16 16"
                      >
                        <path d="M1.385 3.323a.5.5 0 0 1 .707 0L8 9.238l5.908-5.915a.5.5 0 0 1 .707.707L8.707 9.945l.21.21a.5.5 0 0 1 0 .707l-.21.21-5.908 5.915a.5.5 0 0 1-.707-.707L7.293 9.945l-.21-.21a.5.5 0 0 1 0-.707l.21-.21L1.385 3.323z" />
                      </svg>
                      {badge}
                    </span>
                  ))}
          ...
          <div className="divide-y divide-gray-200">
            ...
            <div className="p-6 grid grid-cols-1 md:grid-cols-3 gap-6">
              ...
              <div className="md:col-span-2 space-y-4">
                ...
                <div>
                  ...
                  <p className="mt-1 text-md text-gray-900 font-mono">
                    {profileData.email || "-"}
                  </p>
                </div>
              </div>
            ...
            <div className="p-6 grid grid-cols-1 md:grid-cols-3 gap-6">
              <div className="md:col-span-1">
                ...
                <p className="mt-1 text-sm text-gray-500">
                  Your college and professional links.
                </p>
              </div>
              <div className="md:col-span-2 space-y-4">
                <div>
          ...
          {isOwnProfile && (
            <div className="flex items-center justify-end gap-x-6 p-6 border-t border-gray-200 mt-4">
              <button
                type="submit"
                disabled={isSaving}
                className="px-5 py-2.5 bg-indigo-600 text-white font-semibold rounded-lg shadow-sm hover:bg-indigo-700 disabled:bg-indigo-400"
              >
